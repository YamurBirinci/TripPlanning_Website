...
function HotelOwner() {
    ...
    return (
        <div className="Background_Rectangle" style={{ height: '800px'}}>
            ...
            <button style={{left: '205px'}} className={`ProfileButton ${activeButton === 'MyHotels' ? 'Active-ProfileButton' : ''}`}
                onClick={() => clickingButton('MyHotels', 'MyHotels-Panel')}>
                <FontAwesomeIcon icon={faHotel}></FontAwesomeIcon> My Hotels
            </button >
            <button style={{left: '205px'}} className={`ProfileButton ${activeButton === 'Reservations' ? 'Active-ProfileButton' : ''}`}
                onClick={() => clickingButton('Reservations', 'Reservations-Panel')}>
                <FontAwesomeIcon icon={faUser}></FontAwesomeIcon> Reservations
            </button>
            <button style={{left: '205px'}} className={`ProfileButton ${activeButton === 'AddHotel' ? 'Active-ProfileButton' : ''}`}
            ...
            {activePanel === 'AddHotel-Panel' && <div className='AddHotel-Panel'>
                <div className="ProfileButton-Panel-Container">
                    <div className="container">
                        <div className="form__group field" style={{ width: '200px', left: '-330px', top: '-105px' }}>
                            <input type="input" className="form__field" style={{ width: '200px', height: '52px'}} placeholder="Star" name="Star" id='Star' required/>
                            <label htmlFor="Star" className="form__label"><FontAwesomeIcon icon={faStar}></FontAwesomeIcon> Star</label>
                        </div>
                        <div className="form__group field" style={{ width: '200px', left: '-265px', top: '-105px' }}>
                            <input type="input" className="form__field" style={{ width: '200px', height: '52px'}} placeholder="Price" name="Price" id='Price' required />
                            <label htmlFor="Price" className="form__label"> $ Price</label>
                        </div>
                    </div>
                    <div className="form__group field" style={{ width: '459px', top: '-290px', left: '650px'}}>
                        <input type="input" className="form__field" style={{ width: '459px'}} placeholder="HotelName" name="HotelName" id='HotelName' required />
                        <label htmlFor="HotelName" className="form__label"><FontAwesomeIcon icon={faHotel}></FontAwesomeIcon> Hotel Name</label>
                    </div>
                    <div className="form__group field" style={{ top: '-365px', left: '85px'}}>
                        <input type="password" className="form__field" style={{ width: '1025px'}} placeholder="Location" name="Location" id='Location' required />
                        <label htmlFor="Location" className="form__label"><FontAwesomeIcon icon={faLocationDot}></FontAwesomeIcon> Location</label>
                    </div>
            ...
            <div>
                <h3 style={{color: '#343434'}}>Rez Month</h3>
            ...
            <div>
                <h3 style={{color: '#343434'}} >Rez Status</h3>
                ...
                <div className='Scrolling-Area' >
                    ...
                    {UserReservations.map(Hotel => 
                        (<div key={Hotel}> <div className="Search-Container" style={{transform: 'scale(0.85)', left: '-70px'}}>
                        <img className='Search-Image' src={Hotel.image} alt={Hotel.image}></img>
                        <div>
                            <div style={{fontWeight: 'bold', fontSize: '28px', marginBottom: '5px'}}>{Hotel.Hotel_Name}</div>
                ...
                <button className="cancel_button" style={{transform: 'scale(0.8)', left: '690px', top: '-45px', fontSize: '22px', width: '250px', height: '50px', borderRadius: '15px'}} onClick={ConfirmToCancel}>Cancel the Reservation!</button> 
            </div>))} </div>  
