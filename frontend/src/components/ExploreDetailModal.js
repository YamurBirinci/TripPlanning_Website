...
const ExploreDetailModal = ({ isOpen, closeModal, placeInfo = {}, placeReviews = [], images = [] }) => {
  ...
  console.log(placeInfo.explore_name);

  const VisiblePanel = () => {setVisibleCommentPanel(true);};
  const UnvisiblePanel = () => {setVisibleCommentPanel(false);};

  const averageRate = placeReviews.length > 0
    ? placeReviews.reduce((total, review) => total + review.rate, 0) / placeReviews.length
    : '';

  // Extract address from one of the reviews, if available
  const address = placeReviews.length > 0 ? placeReviews[0].address : placeInfo.address;
  ...
  return ReactDOM.createPortal(
    <div className="modal-overlay">
      ...
      <div className="modal-content">
        ...
        <h1 style={{ marginTop: '20px', left: '46px', position: 'relative' }}>{placeInfo.explore_name}
        ...
        <p style={{ left: '60px', position: 'relative', color: 'black', width: '762px', marginTop: '-45px' }}>{address}</p>
        <button className='Comment-Button' onClick={VisiblePanel} style={{ marginTop:'-25px', width: '770px', position: 'relative', left: '30px', borderBottom: '2px solid #A6A6A6', paddingBottom: '15px', textAlign: 'right' }}>
          <FontAwesomeIcon icon={faCommentDots} /> Comment
        </button>

        {visibleCommentPanel && (
          <div className='Comment-Panel' style={{ top: '30px', position: 'relative', width: '770px', left: "40px", height: '300px', marginBottom: '20px' }}>
        ...
        {placeReviews.map((review, index) => (
          <div key={index}>
            <div style={{ width: '800px', padding: '10px' }}>
              <div className='Comment-Box'>{review.review_text}
